...
deliveries <- d %>% filter(match_id >= 577 & match_id <= 11415)
table(matches$season)

#### Change Data type
d$match_id <- as.factor(d$match_id)
...
s_team <- c("SRH","RCB","MI","RPS","GL","KKR","KXIP","DD","CSK","RR","DC","KTK","PW","RPS","DCs")
...
Season <- data.frame(season=c(2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019),
                     T_winner=c("Rajasthan Royals","Deccan Chargers","Chennai Super Kings","Chennai Super Kings",
...
ggplot(batting_TW,aes(x=season,y=runs,colour=batting_team,fill=batting_team))+
  ...
  geom_text(aes(label=s_team,hjust=-.45, colour="green"))+
